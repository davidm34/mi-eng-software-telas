...
type StatusOnibus = "ativo" | "manutencao" | "inativo";
type FiltroStatus = "todos" | StatusOnibus;

interface OnibusFrota {
  id: string;
  placa: string;
  modelo: string;
  ano: number;
...
const FROTA_INICIAL: OnibusFrota[] = [
  {
    id: "BUS-001",
    placa: "JLS-1020",
...
export default function PaginaAdmin() {
  ...
  const [frota, setFrota] = useState<OnibusFrota[]>(FROTA_INICIAL);
  ...
  const abrirTelaCadastro = (onibus?: OnibusFrota) => {
    if (!onibus) {
      window.alert("Protótipo: abrir tela de Cadastro de Ônibus.");
      return;
    }

    window.alert(`Protótipo: abrir edição do ônibus ${onibus.placa}.`);
  };
